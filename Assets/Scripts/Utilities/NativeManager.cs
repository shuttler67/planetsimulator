...
namespace Unity.Collections
{
    public class NativeManager : INativeManager
    {
        ...
        private Queue<JobHandle> tempJobDependencyQueue = new Queue<JobHandle>();
        ...
        public void DisposeTempJobs()
        {
            int count = tempJobDisposeQueue.Count;
            for (int i = 0; i < count; i++)
            {
                tempJobDependencyQueue.Dequeue().Complete();
                tempJobDisposeQueue.Dequeue().Dispose();
            }
        }

        public void DisposePersistents()
        {
        ...
        public void ProcessDisposables(JobHandle jobHandle)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                switch (allocator)
                {
                    ...
                    case Allocator.TempJob:
                        tempJobDependencyQueue.Enqueue(jobHandle);
